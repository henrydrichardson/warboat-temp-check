...
public class MenuController : MonoBehaviour {
	...
	private float buttonAnimationSpeed = 8;		//speed on animation effect when tapped on button
	private bool canTap = true;					//flag to prevent double tap

	public GameObject scoreLabel;				//score text on menu scene
	...
	void Awake (){
		...
		//prevent getting stuck in menu scene
		Time.timeScale = 1.0f;
		Time.fixedDeltaTime = 0.02f;

		//show the best score in menu scene
		scoreLabel.GetComponent<TextMesh>().text = PlayerPrefs.GetInt("BestScore", 0).ToString();
	}
